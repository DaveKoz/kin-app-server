...
from kinappserver.stellar import send_kin, send_kin_with_payment_service
...
def balance_api():
    ...
    keypair = Keypair()
    balance['base_seed']['kin'] = app.kin_account.get_kin_balance(keypair.address_from_seed(base_seed).address())
    index = 0
    for channel in channel_seeds:
        # seeds only need to carry XLMs
        balance['channel_seeds'][index] = {'kin': 0}
        balance['channel_seeds'][index]['kin'] = app.kin_account.get_kin_balance(keypair.from_seed(channel).address())
        index = index + 1
